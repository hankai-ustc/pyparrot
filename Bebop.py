...
class BebopSensors:
    ...
    def update(self, sensor_name, sensor_value, sensor_enum):
        ...
        # add by hankai
        if (sensor_name =="FlatTrimChanged"):
            self.flatTrimCompleted = True
            print ("Successful Trim")
        if self.drone !=None and self.drone.vision !=None:
            if  self.drone.vision.status == True:
                if (sensor_name == "GpsLocationChanged_altitude"):
                    self.drone.vision.latitudeValue.setText(str(self.sensors_dict["GpsLocationChanged_latitude"]))
                    self.drone.vision.longitudeValue.setText(str(self.sensors_dict["GpsLocationChanged_longitude"]))
                    self.drone.vision.altitudeValue.setText(str(self.sensors_dict["GpsLocationChanged_altitude"]))
                if (sensor_name== "BatteryStateChanged_battery_percent"):
                    self.drone.vision.batteryValue.setText(str(self.sensors_dict["BatteryStateChanged_battery_percent"]))
                if (sensor_name=="SpeedChanged_speedX"):
                    self.drone.vision.speedxValue.setText(str(self.sensors_dict["SpeedChanged_speedX"]))
                if (sensor_name=="SpeedChanged_speedY"):
                    self.drone.vision.speedyValue.setText(str(self.sensors_dict["SpeedChanged_speedY"]))
                if (sensor_name=="SpeedChanged_speedZ"):
                    self.drone.vision.speedzValue.setText(str(self.sensors_dict["SpeedChanged_speedZ"]))
...
class Bebop:
    ...
    def __init__(self):
        ...
        self.sensors = BebopSensors(self)
    ...
    def fly_no_duration(self,roll, pitch, yaw, vertical_movement):
        my_roll = self._ensure_fly_command_in_range(roll)
        my_pitch = self._ensure_fly_command_in_range(pitch)
        my_yaw = self._ensure_fly_command_in_range(yaw)
        my_vertical = self._ensure_fly_command_in_range(vertical_movement)
        command_tuple = self.command_parser.get_command_tuple("ardrone3", "Piloting", "PCMD")
        self.drone_connection.send_fly_command(command_tuple, my_roll, my_pitch, my_yaw, my_vertical)
    ...
    def set_video_stream_resolutions(self,mode="rec1080_stream480"):
        fixed_mode = mode.lower()
        if (fixed_mode not in ("rec1080_stream480","rec720_stream720")):
            print("Error: %s is not a valid stream resolution mode.  Must be one of %s" % (mode, "rec1080_stream480,rec720_stream720"))
            print("Ignoring command and returning")
            return False
        (command_tuple, enum_tuple) = self.command_parser.get_command_tuple_with_enum("ardrone3",
                                                                                    "PictureSettings", "VideoResolutions", mode)
        return self.drone_connection.send_enum_command_packet_ack(command_tuple, enum_tuple)

    def set_vide_stream_framerate(self,rate="25_FPS"):
        frameRate = rate.upper()
        if(frameRate not in ("24_FPS","25_FPS","30_FPS")):
            print("Error: %s is not a valid stream resolution mode.  Must be one of %s" % (frameRate, "24_FPS,25_FPS,30_FPS"))
            return False
        (command_tuple, enum_tuple) = self.command_parser.get_command_tuple_with_enum("ardrone3",
                                                                                   "PictureSettings", "VideoFramerate", frameRate)
        return self.drone_connection.send_enum_command_packet_ack(command_tuple, enum_tuple)
